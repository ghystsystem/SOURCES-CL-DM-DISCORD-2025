...
const fileUrl = 'https://raw.githubusercontent.com/Ghyst Services/Multi-tool/main/src/index.ts';

async function checarUpdates(versionAtual: string): Promise<boolean> {
  try {
    const resposta = await fetch("https://api.github.com/repos/Ghyst Services/Multi-tool/releases/latest");
...
const cleanMessages = async (id: string, isBottomToTop: boolean) => {
  ...
  try {
        ...
        for (const message of sortedMessages) {
          if (!message.system && message.author.id === client.user?.id) {
            try {
              ...
              count++;
              userMessagesCount++;
              console.log(`     [ + ] Deletando ${userMessagesCount} mensagem do usuário em: ${
                channel instanceof DMChannel ? user?.tag : (channel as TextChannel).name
              }`);
              
              await new Promise(resolve => setTimeout(resolve, 100));
            } catch (deleteError) {
              console.log(`     [ ! ] Erro ao deletar mensagem: ${deleteError}`);
              continue; 
            }
          }
          lastId = message.id;
    ...
    if (failedAttempts >= maxFailedAttempts) {
      console.log('     [x] Número máximo de tentativas alcançado. Algumas mensagens podem não ter sido deletadas.');
    }
...
const clearOpenDMs = async () => {
  ...
  for (const dm of dms.values()) {
    ...
    totalDMsProcessed++;
    console.log(`     [i] Progresso: ${totalDMsProcessed}/${totalDMs} DMs processadas`);
  }

  console.log('     [✓] Processo de limpeza finalizado completamente.');
  startCountdown(5);
};
